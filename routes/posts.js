...
router.get('/show/:id', function(req, res, next) {
	var post = db.collection('categories').findOne({'_id' : req.params.id});
	  if(post){ 
		res.render('show',{
  			'post': post
		  });
...
router.post('/add', upload.single('mainimage'), function(req, res, next) {
  // Get Form Values
	...
	if(errors){
		...
		db.collection('posts').insert(data , function(err,collection){
			if(err){
				res.send(err);
			} else {
...
router.post('/addcomment', function(req, res, next) {
  ...
  var commentdate = new Date();
	...
	if(errors){
		 var post = db.collection('posts').findOne({'_id' : postid});
			res.render('show',{
				"errors": errors,
				"post": post
			});
